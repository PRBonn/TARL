...
class TemporalKITTISet(Dataset):
    ...
    def _get_augmented_item(self, index):
        ...
        # pset = np.delete(points_set, pcd_parse_idx, axis=0)
        # segs = np.delete(segments, pcd_parse_idx, axis=0)
        # visualize_pcd_clusters(pset, segs)
    ...
    def _get_item(self, index):
        p1 = np.fromfile(self.points_datapath[index][0], dtype=np.float32)
        p1 = p1.reshape((-1, 4))

        p2 = np.fromfile(self.points_datapath[index][-1], dtype=np.float32)
        p2 = p2.reshape((-1, 4))

        if self.split != 'test':
            label_t = self.points_datapath[index][0].replace('velodyne', 'labels').replace('.bin', '.label')
            label_t = np.fromfile(label_t, dtype=np.uint32).reshape((-1))
            label_t = label_t & 0xFFFF
            label_t = np.vectorize(learning_map.get)(label_t)
            s1 = np.expand_dims(label_t, axis=-1)

            label_tn = self.points_datapath[index][-1].replace('velodyne', 'labels').replace('.bin', '.label')
            label_tn = np.fromfile(label_tn, dtype=np.uint32).reshape((-1))
            label_tn = label_tn & 0xFFFF
            label_tn = np.vectorize(learning_map.get)(label_tn)
            s2 = np.expand_dims(label_tn, axis=-1)

            # unlabeled = sem_labels[:,0] == 0
        ...
        return (torch.from_numpy(coord_t), torch.from_numpy(feats_t), cluster_t, 0), \
                (torch.from_numpy(coord_tn), torch.from_numpy(feats_tn), cluster_tn, len(self.points_datapath[index])-1)
